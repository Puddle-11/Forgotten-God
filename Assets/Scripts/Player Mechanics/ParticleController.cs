...
public class ParticleController : MonoBehaviour
{
	...
	public bool RemoveParticle(int _index, float _deletionTime)
	{
		if (IsIndexValid(_index))
		{
			if (_deletionTime == -1)
			{
				Destroy(particles[_index].gameObject,
				        particles[_index].main.duration + particles[_index].main.startLifetime.Evaluate(0) +
				        particles[_index].main.startLifetime.Evaluate(1));
			}
			else
			{
	...
	public ParticleSystem GetParticle(int _index)
	{
		if (IsIndexValid(_index))
		{
	...
	{
		if (particles != null && particles.Count > 0 && _index >= 0 && _index < particles.Count)
		{
			if (particles[_index] == null)
			{
				Debug.LogWarning("Particle Not assigned at index " + _index + " On object: " + gameObject.name +
	...
	public int AddParticle(ParticleSystem _newPS)
	{
		particles.Add(_newPS);
